...
class DriverContext:
    ...
    keyboard_open: bool = False               # True après ui_type sans submit, False après ui_back
    ...
    if uv:
        ...
        if ok_find and el:
            ...
            if ok_click and matched_item:
                ...
                # Click sur un input : le clavier va s'ouvrir. Sinon, il se ferme.
                ctx.keyboard_open = ctx.last_clicked_was_editable
            return json.dumps(
...
async def tool_ui_type(ctx: DriverContext, text: str, submit: bool = False) -> str:
    ...
    # Tracker l'état du clavier : sans submit, il reste ouvert ; avec submit, l'IME le ferme.
    ctx.keyboard_open = not submit
...
async def tool_ui_back(ctx: DriverContext) -> str:
    ...
    if ctx.session_id:
        ...
        try:
            ...
            if r.status_code in (200, 204):
                ...
                ctx.keyboard_open = False  # back ferme le clavier (ou navigue back, dans tous les cas le clavier disparaît)
                return json.dumps(
    ...
    async with ctx.lock:
        for label in BACK_TARGETS:
            ...
            if ok_click:
                ctx.keyboard_open = False  # click sur back ferme aussi le clavier
                return json.dumps(
...
async def _auto_close_keyboard_if_needed(ctx: DriverContext, action_label: str) -> bool:
    """
    Si le clavier est ouvert, envoie un back Appium avant toute action gesture (swipe/scroll).
    Évite le glide-typing parasite qui tape des caractères pendant le swipe.
    Retourne True si on a fermé le clavier, False sinon.
    """
    if not ctx.keyboard_open:
        return False
    if DEBUG_TOOLS:
        print(f"  [AUTO-BACK] [{ctx.device_id}] keyboard open detected before {action_label}, "
              "sending Appium /back to close it")
    if ctx.session_id:
        try:
            url = f"{APPIUM_SERVER_URL.rstrip('/')}/session/{ctx.session_id}/back"
            async with httpx.AsyncClient(verify=False, timeout=10.0) as client:
                r = await client.post(url, json={})
            if r.status_code in (200, 204):
                ctx.keyboard_open = False
                return True
        except Exception as e:
            if DEBUG_TOOLS:
                print(f"  [AUTO-BACK] HTTP /back failed: {e}")
    # Fallback : on marque comme fermé même si l'appel a foiré, pour éviter de boucler
    ctx.keyboard_open = False
    return False


async def tool_ui_swipe(ctx: DriverContext, direction: str = "down") -> str:
    # Ferme le clavier si encore ouvert (évite le glide-typing parasite)
    await _auto_close_keyboard_if_needed(ctx, "ui_swipe")
...
async def tool_scroll_to_element(ctx: DriverContext, target_text: str) -> str:
    # Ferme le clavier si encore ouvert (évite le glide-typing parasite)
    await _auto_close_keyboard_if_needed(ctx, "scroll_to_element")
    async with ctx.lock:
